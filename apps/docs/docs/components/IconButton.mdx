...
Icon-only buttons require an `aria-label`. Loading forces the button disabled and replaces its content with a progress indicator; set `type="button"` in forms.
