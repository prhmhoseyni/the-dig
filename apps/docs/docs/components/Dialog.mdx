...
import DialogScroll from "../../src/components/DialogScroll";
import DialogSize from "../../src/components/DialogSize";
...
<Dialog open={open} onClose={() => setOpen(false)} width="lg">
  <Dialog.Header className="flex justify-between items-center">
    عنوان تست نمونه
    <X onClick={() => setOpen(false)} className="absolute end-6 cursor-pointer" size={18} />
  </Dialog.Header>
  ...
  <Dialog.Footer className="flex items-center justify-end gap-4">
    <Button variant="tinted" color="gray" onClick={() => setOpen(false)}>
      انصراف
    </Button>
    <Button color="brand" variant="contained" onClick={() => setOpen(false)}>
      تایید
    </Button>
  </Dialog.Footer>
</Dialog>
...
```bash
npx the-dig@latest add Dialog
```

## Props

`Dialog` is controlled through `open` and `onClose`. Compose content with `Dialog.Header`, `Dialog.Body`, and `Dialog.Footer`; `Dialog.Body` accepts `scroll="PAPER" | "BODY"`. Add an accessible title/description and verify keyboard dismissal and focus restoration in the consuming application.
