...
```bash
npx the-dig@latest add SegmentControl
```

## Props

Use direct `SegmentControl.Item` children. The parent controls `value`; item selection and `selectedValue` are injected internally. Add keyboard and radiogroup/tab semantics in the consuming UI.
