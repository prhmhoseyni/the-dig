    ...
    <button
      className={clsx(
        ...
        props.active
          ...
          : "text-prose-primary hover:bg-gray-200",
      )}
      {...rest}
    >
      {children}
...
export default function Pagination(props: PaginationProps) {
  return (
    <div className="w-fit flex items-center gap-px border border-gray-300 rounded-lg p-px">
      ...
      {![1, 2, 3].includes(props.page) && (
        <PaginationButton> ... </PaginationButton>
      )}

      {![1, 2].includes(props.page) && (
        <PaginationButton onClick={() => props.onChange(props.page - 1)}>
          {toPersianDigits(props.page - 1)}
        </PaginationButton>
      )}

      {![1, props.total].includes(props.page) && (
        <PaginationButton active>
          {toPersianDigits(props.page)}
        </PaginationButton>
      )}
      ...
      {![props.total, props.total - 1, props.total - 2].includes(
        props.page,
      ) && <PaginationButton> ... </PaginationButton>}
